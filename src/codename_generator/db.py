...
CREATE TABLE IF NOT EXISTS logs (
    ...
    action    TEXT NOT NULL CHECK (action IN ('added', 'assigned')),
...
def insert_codename(conn: sqlite3.Connection, data: dict) -> int:
    ...
    cur = conn.execute(
        """INSERT INTO codename_inventory (name, name_en, name_zh, theme, sub_theme, brief)
           VALUES (:name, :name_en, :name_zh, :theme, :sub_theme, :brief)""",
        data,
    )
...
def get_assignment_by_project(conn: sqlite3.Connection, project_name: str) -> Optional[dict]:
    ...
    row = conn.execute(
        """SELECT a.*, c.name AS codename_name
...
def get_all_assignments(conn: sqlite3.Connection) -> list[dict]:
    ...
    sql = """SELECT a.id, a.codename_id, c.name AS codename_name,
